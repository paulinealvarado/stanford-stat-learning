...
# K-Nearest Neighbors
Different from other model-fitting functions. Forms prediction using single command with four inputs. Predicts class of a test observation by identifying observations nearest to it with scale being important. cbind() Lag1 and Lag2 into two matrices, training and test sets.  
  
## 4 inputs to knn()   
train.X = matrix containing predictors associated with training data
test.X = matrix containing predictors associated with data used to make predictions
train.Direction = vector containing class labels for training observations
...
## knn() function
K = 1 not good because 50% accuracy. Maybe too flexible fit for the data.
```{r echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}

# Set random seed to ensure reproducibility. Several observations may be tied as nearest neighbors
set.seed(1)
...
K = 3 resulted in slight improvement in accuracy. Thus, not correct model.
```{r echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
## Load data set  
85 predictors measuring demographic characteristics for 5822 individuals. DV is Purchase, indicates individual purchased caravan insurance policy. Only 6% purchased caravan insurance.
```{r echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
## Standardize Data
All variables are given mean of zero and SD of 1 to be on comparable scale.
```{r echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}

standardized.X = scale(Caravan[,-86]) # exclude response variable 
var(Caravan[,1])
var(Caravan[,2])
...
## Split observation into test set (first 1,000 observations) and training set (rest).
```{r echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
## Fit KNN model on training  data w/ K = 1
KNN test set = ~12% error rate 
Since 6% purchased insurance -> error rate down to 6% by always predicting "No"  
```{r echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
# Evaluate perfomance on test data for K = 1
Result: 11.7% customers purchase insurance data
```{r echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
## Fit KNN model on training  data w/ K = 3 and K = 5
Results: KNN is finding real patterns
```{r echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
# 50% threshold: Only 7% will purchase
glm.probs = predict(glm.fits, Caravan[test,], type = "response")
glm.pred = rep("No", 1000)
glm.pred[glm.probs > .5] = "Yes"
table(glm.pred, test.Y)

# 25% threshold: 33% will purchase
glm.probs = predict(glm.fits, Caravan[test,], type = "response")
glm.pred = rep("No", 1000)
